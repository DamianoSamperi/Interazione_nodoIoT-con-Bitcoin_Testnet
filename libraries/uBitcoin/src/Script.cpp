...
void Script::fromAddress(const char * address){
    ...
    for(int i=0; i<unetworks_len; i++){
        if(memcmp(address, unetworks[i]->bech32, strlen(unetworks[i]->bech32))==0){
            type = P2WPKH;
    ...
    if(type == P2WPKH){
        ...
        uint8_t prog[32];
        size_t prog_len = 0;
        int r = segwit_addr_decode(&ver, prog, &prog_len, unetwork->bech32, address);
        ...
        for(int i=0; i<unetworks_len; i++){
            if(addr[0]==unetworks[i]->p2pkh){
                type = P2PKH;
                unetwork = unetworks[i];
                break;
            }
            if(addr[0]==unetworks[i]->p2sh){
                type = P2SH;
                unetwork = unetworks[i];
                break;
            }
...
size_t Script::address(char * buffer, size_t len, const Unetwork * unetwork) const{
    memset(buffer, 0, len);
    if(type() == P2PKH){
        uint8_t addr[21];
    ...
    if(type() == P2WPKH || type() == P2WSH){
        char address[76] = { 0 };
        segwit_addr_encode(address, unetwork->bech32, scriptArray[0], scriptArray+2, scriptArray[1]);
        size_t l = strlen(address);
        if(l > len){
            return 0;
        }
...
String Script::address(const Unetwork * unetwork) const{
    char buffer[100] = { 0 };
    size_t l = address(buffer, sizeof(buffer), unetwork);
    if(l == 0){
        return String("");
    }
