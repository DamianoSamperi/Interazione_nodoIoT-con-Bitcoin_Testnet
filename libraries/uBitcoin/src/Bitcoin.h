...
public:
     ...
     */
    int legacyAddress(char * addr, size_t len, const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    /**
     *  \brief Fills `addr` with native segwit address (P2WPKH, `bc1...` for mainnet)
     */
    int segwitAddress(char * addr, size_t len, const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    /**
     *  \brief Fills `addr` with nested segwit address (P2SH-P2WPKH, `3...` for mainnet)
     */
    int nestedSegwitAddress(char * addr, size_t len, const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    /**
     *  \brief Alias for `legacyAddress`
     */
    int address(char * address, size_t len, const Unetwork * unetwork = &DEFAULT_UNETWORK) const{ return legacyAddress(address, len, unetwork); };
#if USE_ARDUINO_STRING
    String legacyAddress(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    String segwitAddress(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    String nestedSegwitAddress(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    String address(const Unetwork * unetwork = &DEFAULT_UNETWORK) const{ return legacyAddress(unetwork); };
#endif
#if USE_STD_STRING
    std::string legacyAddress(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    std::string segwitAddress(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    std::string nestedSegwitAddress(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
    std::string address(const Unetwork * unetwork = &DEFAULT_UNETWORK) const{ return legacyAddress(unetwork); };
#endif
    /**
...
public:
    PrivateKey();
    PrivateKey(const uint8_t secret_arr[32], bool use_compressed = true, const Unetwork * net = &DEFAULT_UNETWORK);
    ...
    /** \brief Pointer to the unetwork to use. Mainnet or Testnet */
    const Unetwork * unetwork;
    ...
    /** \brief Alias for .publicKey().address(unetwork) */
    int address(char * address, size_t len) const;
    /** \brief Alias for .publicKey().legacyAddress(unetwork) */
    int legacyAddress(char * address, size_t len) const;
    /** \brief Alias for .publicKey().segwitAddress(unetwork) */
    int segwitAddress(char * address, size_t len) const;
    /** \brief Alias for .publicKey().nestedSegwitAddress(unetwork) */
...
public:
    ...
    HDPrivateKey(const uint8_t secret[32], const uint8_t chain_code[32],
                 ...
                 const Unetwork * unetwork = &DEFAULT_UNETWORK,
                 ScriptType key_type = UNKNOWN_TYPE);
    HDPrivateKey(const char xprvArr[]);
    HDPrivateKey(const char * mnemonic, size_t mnemonicSize, const char * password, size_t passwordSize, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL);
#if USE_STD_STRING
    HDPrivateKey(std::string mnemonic, std::string password, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL);
#endif
#if USE_ARDUINO_STRING
    HDPrivateKey(String mnemonic, String password, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL);
#endif
/*    HDPrivateKey(const HDPrivateKey &other):HDPrivateKey(  // copy
        other.num, other.chainCode, other.depth,
        other.parentFingerprint, other.childNumber, other.unetwork, other.type){};
*/
    ~HDPrivateKey();
    ...
    int fromSeed(const uint8_t * seed, size_t seedSize, const Unetwork * unetwork = &DEFAULT_UNETWORK);
    // int fromSeed(const uint8_t seed[64], const Unetwork * unetwork = &DEFAULT_UNETWORK);
    int fromMnemonic(const char * mnemonic, size_t mnemonicSize, const char * password, size_t passwordSize, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL);
    int fromMnemonic(const char * mnemonic, const char * password, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL){
        return fromMnemonic(mnemonic, strlen(mnemonic), password, strlen(password), unetwork, progress_callback);
    }
#if USE_STD_STRING
    int fromMnemonic(std::string mnemonic, std::string password, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL);
#endif
#if USE_ARDUINO_STRING
    int fromMnemonic(String mnemonic, String password, const Unetwork * unetwork = &DEFAULT_UNETWORK, void (*progress_callback)(float) = NULL);
#endif
...
public:
    ...
    HDPublicKey(const uint8_t point[64], const uint8_t chain_code[32],
                 ...
                 const uint8_t parent_fingerprint_arr[4] = NULL,
                 uint32_t childnumber = 0,
                 const Unetwork * net = &DEFAULT_UNETWORK,
                 ScriptType key_type = UNKNOWN_TYPE);
...
public:
    ...
    size_t address(char * buffer, size_t len, const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
#if USE_ARDUINO_STRING
    String address(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
#endif
#if USE_STD_STRING
    std::string address(const Unetwork * unetwork = &DEFAULT_UNETWORK) const;
#endif
...
public:
    ...
    size_t address(char * addr, size_t len, const Unetwork * unetwork = &DEFAULT_UNETWORK) const{ return scriptPubkey.address(addr, len, unetwork); };
#if USE_ARDUINO_STRING
    String address(const Unetwork * unetwork = &DEFAULT_UNETWORK) const{ return scriptPubkey.address(unetwork); };
#endif
#if USE_STD_STRING
    std::string address(const Unetwork * unetwork = &DEFAULT_UNETWORK) const{ return scriptPubkey.address(unetwork); };
#endif
