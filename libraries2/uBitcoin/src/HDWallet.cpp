...
HDPrivateKey::HDPrivateKey(const char * mnemonic, size_t mnemonicSize, const char * password, size_t passwordSize, const Unetwork * net, void (*progress_callback)(float)){
    init();
    fromMnemonic(mnemonic, mnemonicSize, password, passwordSize, net, progress_callback);
}
#if USE_ARDUINO_STRING || USE_STD_STRING
HDPrivateKey::HDPrivateKey(String mnemonic, String password, const Unetwork * net, void (*progress_callback)(float)){
    init();
...
size_t HDPrivateKey::to_bytes(uint8_t * arr, size_t len) const{
    ...
    switch(type){
        case P2WPKH:
            memcpy(hex, unetwork->zprv, 4);
            break;
        case P2SH_P2WPKH:
            memcpy(hex, unetwork->yprv, 4);
            break;
        case P2WSH:
            memcpy(hex, unetwork->Zprv, 4);
            break;
...
size_t HDPrivateKey::from_stream(ParseStream *s){
    ...
    if(bytes_parsed+bytes_read == 4){
        bool found = false;
        for(int i=0; i<unetworks_len; i++){
            if(memcmp(prefix, unetworks[i]->xprv, 4)==0){
                type = UNKNOWN_TYPE;
                found = true;
                unetwork = unetworks[i];
                break;
            }else if(memcmp(prefix, unetworks[i]->yprv, 4)==0){
                type = P2SH_P2WPKH;
                found = true;
                unetwork = unetworks[i];
                break;
            }else if(memcmp(prefix, unetworks[i]->zprv, 4)==0){
                type = P2WPKH;
                found = true;
                unetwork = unetworks[i];
                break;
...
int HDPrivateKey::fromSeed(const uint8_t * seed, size_t seedSize, const Unetwork * net){
    init();
    uint8_t raw[64] = { 0 };
    SHA512 sha;
...
// int HDPrivateKey::fromSeed(const uint8_t seed[64], const Unetwork * net){
//     fromSeed(seed, 64);
// }
int HDPrivateKey::fromMnemonic(const char * mnemonic, size_t mnemonicSize, const char * password, size_t passwordSize, const Unetwork * net, void (*progress_callback)(float)){
    init();
    uint8_t seed[64] = { 0 };
...
#if USE_ARDUINO_STRING || USE_STD_STRING
int HDPrivateKey::fromMnemonic(String mnemonic, String password, const Unetwork * net, void (*progress_callback)(float)){
...
int HDPrivateKey::xpub(char * arr, size_t len) const{
    ...
    switch(type){
        case P2WPKH:
            memcpy(hex, unetwork->zpub, 4);
            break;
        case P2SH_P2WPKH:
            memcpy(hex, unetwork->ypub, 4);
            break;
        case P2WSH:
            memcpy(hex, unetwork->Zpub, 4);
            break;
...
HDPublicKey HDPrivateKey::xpub() const{
    ...
    return HDPublicKey(p.point, chainCode, depth, parentFingerprint, childNumber, unetwork, type);
}
    ...
    if(hardened){
        ...
        if(depth == 1){
            if(index == (HARDENED_INDEX+1)){
                child.unetwork = &Testnet;
            }
            if(index == HARDENED_INDEX){
                child.unetwork = &Mainnet;
            }
        }
...
size_t HDPublicKey::from_stream(ParseStream *s){
    ...
    if(bytes_parsed+bytes_read == 4){
        bool found = false;
        for(int i=0; i<unetworks_len; i++){
            if(memcmp(prefix, unetworks[i]->xpub, 4)==0){
                type = UNKNOWN_TYPE;
                found = true;
                unetwork = unetworks[i];
                break;
            }else if(memcmp(prefix, unetworks[i]->ypub, 4)==0){
                type = P2SH_P2WPKH;
                found = true;
                unetwork = unetworks[i];
                break;
            }else if(memcmp(prefix, unetworks[i]->zpub, 4)==0){
                type = P2WPKH;
                found = true;
                unetwork = unetworks[i];
                break;
...
int HDPublicKey::address(char * addr, size_t len) const{
    switch(type){
        case P2WPKH:
            return PublicKey::segwitAddress(addr, len, unetwork);
        case P2SH_P2WPKH:
            return PublicKey::nestedSegwitAddress(addr, len, unetwork);
        case P2PKH:
            return PublicKey::legacyAddress(addr, len, unetwork);
        default:
            return PublicKey::segwitAddress(addr, len, unetwork);
    }
}
...
String HDPublicKey::address() const{
    switch(type){
        case P2WPKH:
            return PublicKey::segwitAddress(unetwork);
        case P2SH_P2WPKH:
            return PublicKey::nestedSegwitAddress(unetwork);
        case P2PKH:
            return PublicKey::legacyAddress(unetwork);
        default:
            return PublicKey::segwitAddress(unetwork);
    }
}
