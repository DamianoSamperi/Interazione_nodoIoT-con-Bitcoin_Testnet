...
int PublicKey::legacyAddress(char * address, size_t len, const Unetwork * unetwork) const{
    memzero(address, len);
    ...
    uint8_t addr[21];
    addr[0] = unetwork->p2pkh;
    memcpy(addr+1, buffer, 20);

    return toBase58Check(addr, 21, address, len);
}
#if USE_ARDUINO_STRING || USE_STD_STRING
String PublicKey::legacyAddress(const Unetwork * unetwork) const{
    char addr[40] = { 0 };
    legacyAddress(addr, sizeof(addr), unetwork);
    return String(addr);
}
#endif
int PublicKey::segwitAddress(char address[], size_t len, const Unetwork * unetwork) const{
    memzero(address, len);
    ...
    segwit_addr_encode(address, unetwork->bech32, 0, hash, 20);
    return 76;
}
#if USE_ARDUINO_STRING || USE_STD_STRING
String PublicKey::segwitAddress(const Unetwork * unetwork) const{
    char addr[76] = { 0 };
    segwitAddress(addr, sizeof(addr), unetwork);
    return String(addr);
}
#endif
int PublicKey::nestedSegwitAddress(char address[], size_t len, const Unetwork * unetwork) const{
    memzero(address, len);
    uint8_t script[22] = { 0 };
    ...
    uint8_t addr[21];
    addr[0] = unetwork->p2sh;
    hash160(script, 22, addr+1);

    return toBase58Check(addr, 21, address, len);
}
#if USE_ARDUINO_STRING || USE_STD_STRING
String PublicKey::nestedSegwitAddress(const Unetwork * unetwork) const{
    char addr[40] = { 0 };
    nestedSegwitAddress(addr, sizeof(addr), unetwork);
    return String(addr);
}
#endif
...
PrivateKey::PrivateKey(const uint8_t * secret_arr, bool use_compressed, const Unetwork * net){
    reset();
...
int PrivateKey::address(char * address, size_t len) const{
    return pubKey.address(address, len, unetwork);
}
int PrivateKey::legacyAddress(char * address, size_t len) const{
    return pubKey.legacyAddress(address, len, unetwork);
}
int PrivateKey::segwitAddress(char * address, size_t len) const{
    return pubKey.segwitAddress(address, len, unetwork);
}
int PrivateKey::nestedSegwitAddress(char * address, size_t len) const{
    return pubKey.nestedSegwitAddress(address, len, unetwork);
}
#if USE_ARDUINO_STRING || USE_STD_STRING
String PrivateKey::address() const{
    return pubKey.address(unetwork);
}
String PrivateKey::legacyAddress() const{
    return pubKey.legacyAddress(unetwork);
}
String PrivateKey::segwitAddress() const{
    return pubKey.segwitAddress(unetwork);
}
String PrivateKey::nestedSegwitAddress() const{
    return pubKey.nestedSegwitAddress(unetwork);
}
#endif
